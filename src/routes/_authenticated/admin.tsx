...
export const Route = createFileRoute("/_authenticated/admin")({
  head: () => ({
    meta: [{ title: "Workspace — simbi" }, { name: "robots", content: "noindex" }],
  }),
    ...
    <div className="min-h-screen bg-background text-foreground flex overflow-hidden">
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-surface border-r border-border flex flex-col transition-transform duration-300 ${mobileMenu ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="p-4 border-t border-border">
          <a href={linkPublico} target="_blank" rel="noreferrer" className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors mb-1">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="h-16 bg-surface border-b border-border flex items-center justify-between px-4 md:px-8">
          ...
          <div className="flex items-center gap-4">
            <a href={linkPublico} target="_blank" rel="noreferrer" className="hidden md:flex items-center gap-2 text-xs font-semibold bg-secondary hover:bg-border px-3 py-1.5 rounded-md transition-colors">
